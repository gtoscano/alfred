...
void log(){
  ...
  opts.frame_max = 131072;
  opts.auth = AmqpClient::Channel::OpenOpts::BasicAuth(username, password);
  redis.hset("opt4cast_log_emo", "countdown_begin","0" );
  redis.hset("opt4cast_log_emo", "countdown_end","0");
  redis.hset("opt4cast_log_gen", "countdown_begin","0" );
  redis.hset("opt4cast_log_gen", "countdown_end","0");
  redis.hset("opt4cast_log_exec", "countdown_begin","0" );
  redis.hset("opt4cast_log_exec", "countdown_end","0");
  redis.hset("opt4cast_log_emo", "counter","0" );
  redis.hset("opt4cast_log_gen", "counter","0" );
  redis.hset("opt4cast_log_exec", "counter","0" );
  //try
  //{
    auto channel = AmqpClient::Channel::Open(opts);
    ...
    //std::clog << "Queue with name '" << queue_name << "' has been declared.\n";
    auto cinfo =  "Queue with name '" << queue_name << "' has been declared.";
    channel->BindQueue(queue_name, EXCHANGE_NAME, "opt4cast_log_emo");
    channel->BindQueue(queue_name, EXCHANGE_NAME, "opt4cast_log_gen");
    channel->BindQueue(queue_name, EXCHANGE_NAME, "opt4cast_log_exec");
    channel->BindQueue(queue_name, EXCHANGE_NAME, "opt4cast_log_nds");
    std::cout<<" [*] Waiting for calls. To exit press CTRL+C\n";

    auto consumer_tag = channel->BasicConsume(queue_name, "",  true, true, true, 10);
    std::vector<std::string> options = { "opt4cast_log_emo", "opt4cast_log_gen",  "opt4cast_log_exec", "opt4cast_log_nds"};

    //std::clog

    auto cinfo =  "Consumer tag: " << consumer_tag;
    while (true)
    {
      auto envelop = channel->BasicConsumeMessage(consumer_tag);
      auto message_payload = envelop->Message()->Body();
      auto routing_key = envelop->RoutingKey();
      if (find(cbegin(options), cend(options), routing_key) == cend(options)) {
        std::cerr << "Invalid option: " << routing_key << ", please use [send | retrieve]\n";
        exit(1);
      }

      std::string variable_name = routing_key; //it can be opt4cast_log_emo, opt4_cast_log_gen, opt4_cast_log_exec
      std::string level;

      if (variable_name == std::string("opt4cast_log_emo")) {
        level = "emo";
      }
      else if (variable_name == std::string("opt4cast_log_gen")) {
        level = "gen";
      }
      else if (variable_name == std::string("opt4cast_log_exec")) {
        level = "exec";
      }
    if(redis.hexists(variable_name, "count_begin") == false)
      redis.hset(variable_name, "count_begin", "0");
    if(redis.hexists(variable_name, "countdown_begin") == false)
      redis.hset(variable_name, "countdown_begin", "0");
    if(redis.hexists(variable_name, "count_end") == false)
      redis.hset(variable_name, "count_end", "0");
    if(redis.hexists(variable_name, "countdown_end") == false)
      redis.hset(variable_name, "countdown_end", "0");
    ...
    //std::vector<std::string> to_retrieve_list;
    //redis.lrange("opt4cast_exec_time", 0, -1, std::back_inserter(to_retrieve_list));

    //redis.hset(variable_name+"_time" , uuid, fmt::format("{}", timestamp));
    //int len = to_retrieve_list.size();
    //for (int i(0); i< len-99; ++i) {
    //  redis.lpop("opt4cast_exec_time");
    //}

      std::vector<std::string> data_list;
      split_str(message_payload, '_', data_list);
      auto action = data_list[0];
      auto uuid = data_list[1];
      std::cout<<"action: "<<action<<std::endl;



    std::string time_ms_str;
    std::vector<std::string> keys;
    redis.hkeys("opt4cast_log_exec_time", std::back_inserter(keys));
    for (auto my_uuid: keys) {
      std::vector<std::string> stored_keys;
      redis.lrange("opt4cast_log_exec_time_zombie_uuid", 0, -1, std::back_inserter(stored_keys));
      if (find(cbegin(stored_keys), cend(stored_keys), my_uuid) == cend(stored_keys)) {
        std::string timestamp_str = *redis.hget("opt4cast_log_exec_time", my_uuid);
        long timestamp_begin = std::stol(timestamp_str);
        int diff_time = (timestamp - timestamp_begin)/60000;
        if (diff_time > 5) {
          redis.rpush("opt4cast_log_exec_time_zombie_uuid", my_uuid);
          redis.rpush("opt4cast_log_exec_time_zombie_min", timestamp_str);
        }
      }
    }


      if (action == std::string("init")) {
        int size = std::stoi(data_list[2]);
        std::cout<<"size: "<<variable_name << "  "<<size<<std::endl;
        redis.hset(variable_name+"_time" , uuid, fmt::format("{}", timestamp));
        if (level == std::string("emo")) {
          int counter = 0;
          counter = std::stoi(*redis.hget("opt4cast_log_gen", "countdown_begin"));
          redis.hset("opt4cast_log_gen", "countdown_begin", fmt::format("{}", counter+size));
          counter = std::stoi(*redis.hget("opt4cast_log_gen", "countdown_end"));
          redis.hset("opt4cast_log_gen", "countdown_end", fmt::format("{}", counter+size));

        }
        else if (level == std::string("gen")) {
          int counter = 0;
          counter = std::stoi(*redis.hget("opt4cast_log_exec", "countdown_begin"));
          redis.hset("opt4cast_log_exec", "countdown_begin", fmt::format("{}", counter+size));
          counter = std::stoi(*redis.hget("opt4cast_log_exec", "countdown_end"));
          redis.hset("opt4cast_log_exec", "countdown_end", fmt::format("{}", counter+size));

          counter = std::stoi(*redis.hget("opt4cast_log_gen", "countdown_begin"));
          redis.hset("opt4cast_log_gen", "countdown_begin", fmt::format("{}", counter-1));

        }

        if(redis.hexists(variable_name, "counter") == false){
          redis.hset(variable_name, "counter", "1");
        }
        else{
          int counter = std::stoi(*redis.hget(variable_name, "counter"));
          redis.hset(variable_name, "counter", fmt::format("{}", counter+1));
        }

      }

      else if (action == std::string("begin")) {
        std::string exec_uuid = data_list[2];
        std::cout<<"exec_uuid "<<exec_uuid<<std::endl;
        std::cout<<"emo_uuid "<<uuid<<std::endl;
        redis.hset(variable_name+"_time" , exec_uuid, fmt::format("{}", timestamp));
        redis.rpush(uuid+"_execs", exec_uuid);
        int counter = std::stoi(*redis.hget(variable_name, "count_begin"))+1;
        redis.hset(variable_name, "count_begin", fmt::format("{}", counter));
        counter = std::stoi(*redis.hget(variable_name, "countdown_begin"))-1;
        redis.hset(variable_name, "countdown_begin", fmt::format("{}", counter));
      }
      else if (action == std::string("end")) {
        std::string exec_uuid = data_list[2];

        std::cout<<"variable_name "<<variable_name<<" exec_uuid: "<<exec_uuid<<std::endl;
        std::cout<<"hget "<<*redis.hget(variable_name+"_time", exec_uuid)<<"\n";
          std::cout<<"hget "<<std::stol(*redis.hget(variable_name+"_time", exec_uuid))<<"\n";
        long timestamp_begin = std::stol(*redis.hget(variable_name + "_time", exec_uuid));
        int diff_time = timestamp - timestamp_begin;
        ...
        redis.rpush("opt4cast_exec_time", fmt::format("{}", diff_time));
        redis.hset(variable_name + "_end_time", exec_uuid, fmt::format("{}", diff_time));
        redis.hdel(variable_name+"_time", exec_uuid);
        int counter = std::stoi(*redis.hget(variable_name, "count_end"))+1;
        redis.hset(variable_name, "count_end", fmt::format("{}", counter));
        counter = std::stoi(*redis.hget(variable_name, "countdown_end"))-1;
        redis.hset(variable_name, "countdown_end", fmt::format("{}", counter));
      }
      else if (action == std::string("terminate")) {
        int timeframe=1000;
        if (level==std::string("gen"))
          timeframe = 1000;
        else
          timeframe = 60000;
        //int size = std::stoi(data_list[2]);
        long timestamp_begin = std::stol(*redis.hget(variable_name + "_time", uuid));
        int diff_time = (timestamp - timestamp_begin)/timeframe;

        std::vector<std::string> to_retrieve_list;
        redis.lrange(variable_name+"_millisec", 0, -1, std::back_inserter(to_retrieve_list));
        int len = to_retrieve_list.size();
        for (int i(0); i< len-99; ++i) {
          redis.lpop(variable_name+"_millisec");
        }
        redis.rpush(variable_name+"_millisec", fmt::format("{}", diff_time));

        if (level == std::string("gen")) {
          int counter = 0;
          counter = std::stoi(*redis.hget("opt4cast_log_gen", "countdown_end"));
          int result = (counter>0)?counter-1:0;
          redis.hset("opt4cast_log_gen", "countdown_end", fmt::format("{}", result));

        }
          if(redis.hexists(variable_name, "counter") == false) {
            redis.hset(variable_name, "counter", "0");
          }
      ...
      else if ( action == std::string("plot")) {
        std::string key_str = "opt4cast_log_plot";
        std::string  key_counter = "opt4cast_log_plot_counter";
        std::string  key_x = "opt4cast_x";
        std::string  key_y = "opt4cast_y";
        auto max_location = 9;
        std::string location;

        if (redis.hexists(key_str, uuid) == false) {
          location = *redis.lpop(key_counter);
          std::cout<<"localtion: "<<location<<"\n";
          std::cout<<"key_stry "<<key_str<< "\n";
          std::cout<<"uuid "<<uuid<<"\n";
          redis.hset(key_str, uuid, location);
          redis.rpush(key_counter,  location);
        }
        else {
          location  = *redis.hget(key_str, uuid);
        }
        //if (redis.hexists(key_str, uuid) == false) {
        redis.del({fmt::format("{}_{}", key_x, location)});
        redis.del({fmt::format("{}_{}", key_y, location)});

        //std::string content = data_list[2];

        //std::vector<std::string> data_list;
        //split_str(content, '_', data_list);
        int nds_size = (int) data_list.size()-2;
        std::cout<<"key"<<key_x<<location<<std::endl;
        for(int i(0); i < nds_size/2; ++i) {
          redis.rpush(fmt::format("{}_{}", key_x, location), fmt::format("{}", data_list[2+i]));
          redis.rpush(fmt::format("{}_{}", key_y, location), fmt::format("{}", data_list[2+i+nds_size/2]));
        }

        //std::vector<std::string> retrieving_queue;
        //redis.lrange(key_x + fmt::format("_{}", location) , 0, -1, std::back_inserter(retrieving_queue));
...
//json core1 = json::parse(core_str);
//json scenario1 = json::parse(scenario_str);
//json geography1 = json::parse(geography_str);

//core1["DataTransferObject"]["Id"] = scenario_id+1;
//core1["DataTransferObject"]["FileId"] =  "cotz";
//std::cout<<core.dump(4)<<"\n";
//std::cout<<scenario.dump(4)<<"\n";
//std::cout<<geography.dump(4)<<"\n";
//std::cout<<solutions<<std::endl;
/*
alfred.create_generation(curr_gen,
                         pop_size,
                         solutions
                         );
alfred.allocation_mode("one");
alfred.eval_pop(curr_gen, "corecast", threshold);
alfred.eval_pop(curr_gen, "model", threshold);
alred.wait_for_pop(curr_gen);
alfred.cout_fx(curr_gen, "corecast");
alfred.cout_fx(curr_gen, "model");
*/


/*"processorfactory": "scenarioprocessorfactory",
"processorservicename": "scenariorunprocessor",
"datatransferobject": { "copyag": [], "copydc": [], "copyde": [], "copymd":
[], "copyny": [], "copypenn": [], "copyva": [], "copywva": [], "copynat": [],
"copysept": [], "copydev": [], "copypol": [], "copysingle": [], "needsrerun":
false, "ispublic": false, "isoptimizationmode": true, "needsvalidation":
false, "id": %i, "statusid": 0, "name": null, "userdata": { "eventtext": "",
"firstname": "", "lastname": "", "notify": [""], "eventkey": "runscenario",
"notifiedby": "", "notifyreason": "", "userid":
"00000000-0000-0000-0000-000000000000"
},
"source": 0, "sourcedataversion": "", "networkpath": "", "fileid": "%s",
"landbmpimportfiles": [], "aimalbmpimportfiles": [], "manurebmpimportfiles":
[], "landpolicybmpimportfiles": []
}
*/
